...
QUERY = f"""
    SELECT
        ...
        sich AS sic,    -- industry code
        at,             -- total assets
        sale,           -- net sales
        ib,             -- income before extraordinary items (net income proxy)
        fopo AS pifo,   -- foreign operating income, renamed for our script
        xrd,            -- R&D expenditure
        dltt,           -- long-term debt total
        emp,            -- employees (thousands)
        NULL AS inco    -- year of incorporation not available in this WRDS table
    FROM
        comp_global_daily.g_funda
    WHERE
    fyear BETWEEN 2015 AND 2020
"""
