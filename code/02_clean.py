...
# -- SME Filter (EU Definition) -----------------------------------------------
# EU Definition: < 250 Mitarbeiter ODER Total Assets <= 43 Mio EUR
# emp in Compustat ist in Tausend → 0.25 = 250 Mitarbeiter
# Firmen mit fehlenden emp-Werten werden behalten (konservative Annahme)
if "emp" in df.columns and "at" in df.columns:
    n_before = len(df)
    sme_mask = (
        (df["emp"] < 0.25) |           # weniger als 250 Mitarbeiter
        (df["at"] <= 43) |             # Total Assets <= 43 Mio EUR
        (df["emp"].isna())             # fehlende emp-Werte behalten
    )
    df = df[sme_mask].copy()
    n_removed = n_before - len(df)
    print(f"\nSME Filter (EU Definition):")
    print(f"  Before: {n_before:,} rows")
    print(f"  After:  {len(df):,} rows (removed {n_removed:,} Grossunternehmen)")
    print(f"  Davon kleine Firmen (emp < 0.25): {(df['emp'] < 0.25).sum():,}")
    print(f"  Davon nach Assets (at <= 43 Mio): {(df['at'] <= 43).sum():,}")
    print(f"  Davon fehlende emp-Werte behalten: {df['emp'].isna().sum():,}")
else:
    print("\nWARNING: emp oder at nicht gefunden — SME Filter übersprungen")
...
key_vars = ["sale", "at", "ib", "dltt", "dlc", "seq", "emp"]
for col in key_vars:
...
print(df_check[["gvkey", "conm", "fyear", "loc", "curcd",
                 "sale", "at", "ib", "dltt", "dlc", "seq", "emp"]].head(10).to_string())